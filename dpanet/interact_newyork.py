...
net=cityflowlib.net("./data/NewYork/28_7/roadnet_28_7.json")
...
savedatadirpath = "./model_save/recorddata_qst3fc10/"
...
class collectband:
    ...
    def funcollect_qrs(self,connection:engine,intersection_idx:int,phase_number:int,new_phase:int,phase_duration:int):
        ...
        prevphasevec[prevphaseidx] = 1
        self.prevaction_dst[intersection_idx].append(prevphasevec)
    def funcollect_qrs_fast(self,connection:engine,intersection_idx:int,phase_number:int,new_phase:int,phase_duration:int,ct:int):
        '''
        - q:waiting queue (12,1)
        - r:running count (12,1)
        - s:segment data (12,4)
        ...
        if self.ct != ct:
            ...
            # waiting_data = connection.get_lane_waiting_vehicle_count()
            # lane_flow_dict: Dict[str, list] = connection.get_lane_vehicles()
            self.flow_speed_dict = connection.get_vehicle_speed()
            self.waiting_data = connection.get_lane_waiting_vehicle_count()
            self.lane_flow_dict = connection.get_lane_vehicles()
            self.vehicle_distance = connection.get_vehicle_distance()
            self.ct = ct
        flow_speed_dict = self.flow_speed_dict
        waiting_data = self.waiting_data
        lane_flow_dict = self.lane_flow_dict
    ...
    def extract_lastwaitstate_imdt(self,connection:engine,intersection_idx):
        ...
        reswaiting = self.datafilterlist[intersection_idx].get_waiting_data(waiting_data)
        return reswaiting
    def extract_lastwaitstate_imdt_fast(self,connection:engine,intersection_idx,ct:int):
        if self.testct != ct:
            self.testwaiting_data = connection.get_lane_waiting_vehicle_count()
            self.testct = ct
        waiting_data = self.testwaiting_data
        reswaiting = self.datafilterlist[intersection_idx].get_waiting_data(waiting_data)
        return reswaiting
    ...
    def extract_stateimdt_qrs(self,connection:engine,intersection_idx:int,dataseq_num:int):
        ...
        if dataseq_num <=1 :
            ...
            return res_tastate,DataSrcActionPrev
        raise ValueError("dataseq_num must be greater than 1")
    def extract_stateimdt_qrs_fast(self,connection:engine,intersection_idx:int,dataseq_num:int,ct:int):
        '''
        - q:waiting queue (12,1)
        - r:running count (12,1)
        - s:segment data (12,4)
...
if __name__=='__main__':
    ...
    record_dirpath = "./labs/qsttmixer3res2_simple_s_hu_ny/records/ny28_7_double4/"
    ...
    cfgdict = {
        "laneChange":False,
        "dir":"./data/NewYork/28_7/",
        "roadnetFile":"roadnet_28_7.json",
        # "flowFile":"anon_3_4_jinan_real_2500.json",
        "flowFile":"anon_28_7_newyork_real_double.json",
        "rlTrafficLight": True
    }
    ...
    traincriticnums = 1
    traincriticcollects = 1
    pretrainnums = 2

    normaltrainnums = 6
    normaltrainepoch = 80
    normaltraincollects = 1
    greedystart_idx_table = [2,5,16,30,55]
    greedyvalue_table = [0.2,0.25,0.25,0.25,0.5]
    gamma_table = [0.3,0.65,0.65,0.65,0.65]
    freq_table = [4,4,4,4,2]
    greedy_default = 0.65
    # normaltrainepoch = 80
    ...
    for datainter in range(bufferlen-traincriticcollects):
        eng = engine.Engine(cfgpath, thread_num=12)
        tq = run_queue.DACS_taskrunframeV4(180,yellow_duration=yellowtime)
        random_12list = [random.randint(1,4) for _ in range(nodenum)]
        init_preaction = [[i,random_12list[i],0,1] for i in range(nodenum)]
        ...
        cb.anode_space = [{"phase":random_12list[i],"phaselist":[1,2,3,4],"target_runtime":0,"current_runtime":0,"currentphaseidx":[1,2,3,4].index(random_12list[i])} for i in range(nodenum)]
        ...
        cb.init_flowrecord()
        tq.process_tasks_limit_finialpro(eng,3600,cb.set_signal,choose_action,cb.step,cb.funcollect_qrs_fast)
    ...
    if traincritic_first == True:
        ...
        for traincritic in range(traincriticnums):
            ...
            for datainter in range(traincriticcollects):
                ...
                cb.config_common_lanesnum(datafilterlist[0]) #选择一个节点的车道数量
                cb.init_flowrecord()
                tq.process_tasks_limit_finialpro_interact_customfunc(eng,3600,cb.set_signal,choose_action,choose_action_epsystem,cb.step,cb.funcollect_qrs_fast,cb.extract_stateimdt_qrs_fast,cb.extract_lastwaitstate_imdt_fast,1,0,1,env.choose_action)
                current_traveltime = eng.get_average_travel_time()
            ...
            for traintimes in range(pretrainnums):
                ...
                #     break
                # currenttimestr_tmp = time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())
                # env.gradarrayoutput(os.path.join(current_record_dir,"grads_"+"_tc{:02d}_{:02d}.npy".format(traincritic,traintimes)))
    ...
    for trainepoch in range(normaltrainepoch):
        ...
        while ck < collect_times:
            ...
            tq.process_tasks_limit_finialpro_interact_customfunc(eng,3600,cb.set_signal,choose_action,choose_action_epsystem,cb.step,cb.funcollect_qrs_fast,cb.extract_stateimdt_qrs_fast,cb.extract_lastwaitstate_imdt_fast,1,greedy,1,env.choose_action)
            current_traveltime = eng.get_average_travel_time()
        ...
        for traintimes in range(normaltrainnums):
            ...
            #     break
            # currenttimestr_tmp = time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime())
            # env.gradarrayoutput(os.path.join(current_record_dir,"grads_"+"_{:02d}_{:02d}.npy".format(trainepoch,traintimes)))
            env.hard_update(env.qnet,env.qnet_target)
            env.hard_update(env.pactor,env.pactor_target)
        testloss = []
        for batch in eval_dataloader:
        ...
        for datainter in range(1):
            ...
            cb.config_common_lanesnum(datafilterlist[0]) #选择一个节点的车道数量
            cb.init_flowrecord()
            tq.process_tasks_limit_finialpro_interact(eng,3600,cb.set_signal,choose_action,cb.step,cb.funcollect_qrs_fast,cb.extract_stateimdt_qrs_fast,1,greedy-0.08,1,env.choose_action)
            current_traveltime = eng.get_average_travel_time()
            print(current_traveltime)
            cb.compiledate()
            dstdata_slice,actionslice,actionprevslice = cb.extractdata_oneagent_simple()
        ...
        for datainter in range(1):
            ...
            cb.config_common_lanesnum(datafilterlist[0]) #选择一个节点的车道数量
            cb.init_flowrecord()
            tq.process_tasks_limit_finialpro_interact(eng,3600,cb.set_signal,choose_action,cb.step,cb.funcollect_qrs_fast,cb.extract_stateimdt_qrs_fast,1,1,1,env.choose_action)
            current_traveltime = eng.get_average_travel_time()
