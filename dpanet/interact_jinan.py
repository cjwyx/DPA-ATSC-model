...
if __name__=='__main__':
    ...
    record_dirpath = "labs/qsttmixer3res_simple_s_hu/records/jinanfc2500_offline_C/"
    ...
    # greedystart_idx_table = [2,5,16,20,40]
    ...
    for datainter in range(bufferlen-traincriticcollects):
        ...
        random_12list = [random.randint(1,4) for _ in range(12)]
        init_preaction = [[i,random_12list[i],0,1] for i in range(12)]
        cb = collectband(nodelist,phasetime_max)
        cb.action_dst = [[] for _ in range(12)]
        cb.state_dst = [[] for _ in range(12)]
        cb.prevaction_dst = [[] for _ in range(12)]
        cb.anode_space = [{"phase":random_12list[i],"phaselist":[1,2,3,4],"target_runtime":0,"current_runtime":0,"currentphaseidx":[1,2,3,4].index(random_12list[i])} for i in range(12)]
